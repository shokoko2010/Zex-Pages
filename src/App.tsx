...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (!envInfo.hasRequiredEnvVars && envInfo.isProduction) {
      console.error('Missing required environment variables in production. Some features may not work properly.');
    }
  }, []);
  ...
  const renderContent = () => {
    ...
    if (currentPath.startsWith('/admin') && appUser.isAdmin) {
      return <AdminPage appUser={appUser} allUsers={allUsers} onLogout={handleLogout} onSettingsClick={() => setIsSettingsModalOpen(true)} theme={theme} onToggleTheme={handleToggleTheme} plans={plans} onSelectTarget={setSelectedTarget} onNavigateToDashboard={() => setCurrentPath('/')} />;
    }
