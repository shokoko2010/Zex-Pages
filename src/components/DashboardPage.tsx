...
const DashboardPage: React.FC<DashboardPageProps> = ({ user, isAdmin, userPlan, managedTarget, allTargets, onChangePage, onLogout, aiClient, stabilityApiKey, onSettingsClick, fetchWithPagination, theme, onToggleTheme, strategyHistory, onDeleteStrategy, onSavePlan }) => {
    ...
    const clearComposer = useCallback(() => {
      ...
      setIncludeInstagram(false); setEditingScheduledPostId(null);
    }, []);
    ...
    const handleFetchProfile = async () => {
      ...
      try {
          ...
          const facebookDataForGemini = { about: pageInfo.about, category: pageInfo.category, contact: pageInfo.emails?.[0] || pageInfo.phone, website: pageInfo.website, address: pageInfo.location?.street || pageInfo.contact_address?.street1, country: pageInfo.location?.country || pageInfo.contact_address?.country };
          if (aiClient) {
              ...
              const fallbackProfile = { ...pageProfile, description: pageInfo.about || pageProfile.description, contactInfo: pageInfo.emails?.[0] || pageInfo.phone || pageProfile.contactInfo, website: pageInfo.website || pageProfile.website, address: pageInfo.location?.street || pageProfile.address, country: pageInfo.location?.country || pageProfile.country, ownerUid: user.uid, members: pageProfile.members.includes(user.uid) ? pageProfile.members : [...pageProfile.members, user.uid] };
  ...
  const syncFacebookData = useCallback(async (target: Target) => {
    ...
    if (syncingTargetId) { showNotification('error', 'مزامنة أخرى قيد التنفيذ، يرجى الانتظار.'); return; }
    ...
    try {
        ...
        showNotification('partial', `(1/4) جلب المجدول...`);
        ...
        showNotification('partial', `(2/4) جلب المنشورات...`);
        ...
        for (let i = 0; i < fbPublishedContent.length; i += batchSize) {
            ...
            if (engagementBatchRequest.length > 0) {
                try {
                    ...
                    engagementData.forEach(res => { if (res && res.code === 200) { const body = JSON.parse(res.body); engagementMap.set(body.id, body); } });
        ...
        const finalPublished = fbPublishedContent.map((post: any) => {
            ...
            return {
                id: post.id, text: post.message || '', publishedAt: new Date(post.created_time), imagePreview: getImageUrl(post),
                analytics: { likes: engagement.likes?.summary?.total_count || 0, comments: engagement.comments?.summary?.total_count || 0, shares: engagement.shares?.summary?.total_count || 0, lastUpdated: new Date().toISOString() },
        ...
        // Comments from Published Posts
        try {
            if (fbPublishedContent.length > 0) {
                const commentsBatchRequest = fbPublishedContent.map(post => ({ method: 'GET', relative_url: `${post.id}/comments?limit=10&fields=from,message,created_time,id` }));
    ...
    const handlePublish = async (postType: PostType) => {
        setComposerError('');
        if (!managedTarget.access_token) { setComposerError('رمز الوصول للصفحة غير صالح.'); showNotification('error', 'رمز الوصول للصفحة غير صالح.'); return; }
        setIsPublishing(true);
        try {
            console.log(`Publishing ${postType}:`, { text: postText, image: selectedImage, scheduled: isScheduled, date: scheduleDate });
    ...
    const handleSaveDraft = async () => {
        if (!postText.trim() && !selectedImage) { showNotification('error', 'لا يمكن حفظ مسودة فارغة.'); return; }
        const newDraft: Draft = { id: `draft_${Date.now()}`, text: postText, hasImage: !!selectedImage, imagePreview: imagePreview || undefined, createdAt: new Date().toISOString() };
    ...
    const handleApprovePost = (postId: string) => showNotification('partial', `الموافقة على المنشور ${postId} (محاكاة)...`);
    const handleRejectPost = (postId: string) => showNotification('partial', `رفض المنشور ${postId} (محاكاة)...`);
    ...
    const handleLoadDraft = (draftId: string) => {
        ...
        if (draft) {
            ...
            if (draft.hasImage && draft.imagePreview) {
                ...
                showNotification('partial', 'تم تحميل نص المسودة. يرجى إعادة تحديد الصورة للنشر.');
            } else {
                setImagePreview(null);
    ...
    const onGeneratePerformanceSummary = async () => {
        if (!aiClient || !publishedPosts.length) { showNotification('error', 'لا توجد بيانات أو عميل AI لإنشاء الملخص.'); return; }
        ...
        try {
            ...
            const calculatedSummary: PerformanceSummaryData = { totalPosts: publishedPosts.length, averageEngagement: publishedPosts.length > 0 ? totalEngagement / publishedPosts.length : 0, growthRate: 0, totalReach: 0, totalEngagement, engagementRate: 0, topPosts: publishedPosts.sort((a, b) => (b.analytics.likes || 0) + (b.analytics.comments || 0) + (b.analytics.shares || 0) - ((a.analytics.likes || 0) + (a.analytics.comments || 0) + (a.analytics.shares || 0))).slice(0, 3), postCount: publishedPosts.length, };
        ...
        const onGenerateDeepAnalytics = async () => {
            if (!aiClient || !publishedPosts.length) { showNotification('error', 'لا توجد بيانات أو عميل AI لإنشاء تحليلات معمقة.'); return; }
        ...
        const onFetchPostInsights = async (postId: string): Promise<any> => {
            ...
            if (!aiClient || !post) { showNotification('error', 'الذكاء الاصطناعي غير متاح أو المنشور غير موجود.'); return null; }
            showNotification('partial', 'جاري جلب رؤى المنشور...');
            try {
                const commentsForInsights: { message: string }[] = inboxItems.filter(i => i.type === 'comment' && i.link?.includes(post.id)).map(c => ({message: c.text}));
        ...
        const rescheduleBulkPosts = (postsToReschedule: BulkPostItem[], strategy: 'even' | 'weekly', settings: WeeklyScheduleSettings): BulkPostItem[] => {
            ...
            if (strategy === 'even') {
                ...
                return updatedPosts.map((post, index) => ({ ...post, scheduleDate: new Date(startDate.getTime() + index * 3 * 60 * 60 * 1000).toISOString() }));
            }
            if (strategy === 'weekly') {
                ...
                while (postIndex < updatedPosts.length) {
                    if (days.includes(currentDate.getDay())) {
                        ...
                        if (scheduleDate < new Date()) scheduleDate.setDate(scheduleDate.getDate() + 7);
        ...
        const onAddPosts = (files: FileList | null, textContent?: string) => {
            if (files) {
                const newPosts: BulkPostItem[] = Array.from(files).map((file, index) => ({ id: `bulk_${Date.now()}_${index}`, text: '', imageFile: file, imagePreview: URL.createObjectURL(file), hasImage: true, scheduleDate: '', targetIds: [managedTarget.id] }));
                setBulkPosts(prev => [...prev, ...newPosts]);
                showNotification('success', `تمت إضافة ${newPosts.length} منشورات للجدولة المجمعة.`);
            } else if (textContent) {
                const newTextPost: BulkPostItem = { id: `bulk_${Date.now()}_text_${Math.random()}`, text: textContent, hasImage: false, scheduleDate: '', targetIds: [managedTarget.id] };
        ...
        const onGeneratePostFromText = async (id: string, text: string) => {
            ...
            try {
                ...
                showNotification('success', 'تم توليد نص المنشور.');
        ...
        const onGenerateImageFromText = async (id: string, text: string, service: 'gemini' | 'stability') => {
            ...
            try {
                ...
                } else { throw new Error("خدمة توليد الصور غير متاحة."); }
                ...
                showNotification('success', `تم توليد الصورة باستخدام ${service}.`);
        ...
        const onScheduleAll = async () => {
          if (bulkPosts.length === 0) { showNotification('error', 'لا توجد منشورات للجدولة.'); return; }
          showNotification('partial', 'جاري جدولة جميع المنشورات...');
          setIsPublishing(true);
          try {
              ...
              setBulkPosts([]);
              showNotification('success', 'تمت جدولة جميع المنشورات بنجاح!');
              await syncFacebookData(managedTarget);
          } catch (error: any) {
        ...
        const onGeneratePlan = async (request: StrategyRequest, images?: any[]) => {
            if (!aiClient) { setPlanError('عميل الذكاء الاصطناعي غير مكوّن.'); return; }
            ...
            try {
                ...
                showNotification('success', 'تم إنشاء الخطة وحفظها في السجل!');
            } catch (e: any) {
        ...
        const onScheduleStrategy = async () => {
            if (!contentPlan || contentPlan.length === 0) { showNotification('error', 'لا توجد خطة لتحويلها.'); return; }
            ...
            try {
                const newBulkItems: BulkPostItem[] = contentPlan.map((item, index) => ({
                    id: `bulk_strategy_${Date.now()}_${index}`, text: `${item.hook}\n${item.headline}\n${item.body}`,
                    hasImage: false, scheduleDate: '', targetIds: [managedTarget.id],
                }));
                ...
                showNotification('success', `تم تحويل ${scheduledBulkItems.length} منشورًا إلى الجدولة المجمعة!`);
                setView('bulk');
            } catch (error: any) {
            ...
            switch (view) {
                ...
                case 'inbox': 
                    return (
                        <InboxPage 
                            ...
                            onReply={async (item: InboxItem, message: string) => {
                              if (!managedTarget.access_token) { showNotification('error', 'رمز الوصول للصفحة مفقود للرد.'); return false; }
                              try {
                                  console.log(`Replying to ${item.authorName} (${item.type}): ${message}`);
                                  await new Promise(resolve => setTimeout(resolve, 1000));
                                  const updatedInboxItems = inboxItems.map(i => i.id === item.id ? { ...i, status: 'replied' as 'replied' } : i);
                      ...
                      onGenerateSmartReplies={async (commentText: string) => {
                        if (!aiClient) { showNotification('error', 'عميل الذكاء الاصطناعي غير مكوّن.'); return []; }
                    ...
                    onFetchMessageHistory={async (conversationId: string) => {
                      if (!managedTarget.access_token) { showNotification('error', 'رمز الوصول مفقود.'); return; }
                      ...
                      try {
                          ...
                          setInboxItems(prevItems => prevItems.map(item => 
                              item.conversationId === conversationId ? { ...item, messages: messages.map((msg: any) => ({ id: msg.id, text: msg.message, from: (msg.from.id === managedTarget.id ? 'page' : 'user'), timestamp: msg.created_time })) } : item
                          ));
