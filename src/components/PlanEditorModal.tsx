...
')}))} 
          ...
          <fieldset className="border p-4 rounded-md dark:border-gray-600">
            ...
            <div className="grid grid-cols-2 gap-4">
                <div className="p-2"><label htmlFor="limit-pages" className="block text-sm font-medium whitespace-normal">عدد الصفحات (-1 للانهائي)</label><input type="number" id="limit-pages" value={draftPlan.limits.pages} onChange={e => handleLimitChange('pages', e.target.value)} className="w-full p-2 border rounded-md dark:bg-gray-700 dark:border-gray-600" /></div>
                <div className="flex items-center p-2 pt-5"><input type="checkbox" id="limit-ai-text" checked={draftPlan.limits.aiText} onChange={e => handleLimitChange('aiText', e.target.checked)} className="h-4 w-4" /><label htmlFor="limit-ai-text" className="mr-2 whitespace-normal">ميزات نص AI</label></div>
                <div className="flex items-center p-2 pt-5"><input type="checkbox" id="limit-ai-image" checked={draftPlan.limits.aiImage} onChange={e => handleLimitChange('aiImage', e.target.checked)} className="h-4 w-4" /><label htmlFor="limit-ai-image" className="mr-2 whitespace-normal">ميزات صور AI</label></div>
                <div className="flex items-center p-2"><input type="checkbox" id="limit-responder" checked={draftPlan.limits.autoResponder} onChange={e => handleLimitChange('autoResponder', e.target.checked)} className="h-4 w-4" /><label htmlFor="limit-responder" className="mr-2 whitespace-normal">الرد التلقائي</label></div>
                <div className="flex items-center p-2"><input type="checkbox" id="limit-planner" checked={draftPlan.limits.contentPlanner} onChange={e => handleLimitChange('contentPlanner', e.target.checked)} className="h-4 w-4" /><label htmlFor="limit-planner" className="mr-2 whitespace-normal">مخطط المحتوى</label></div>
                <div className="flex items-center p-2"><input type="checkbox" id="limit-bulk" checked={draftPlan.limits.bulkScheduling} onChange={e => handleLimitChange('bulkScheduling', e.target.checked)} className="h-4 w-4" /><label htmlFor="limit-bulk" className="mr-2 whitespace-normal">الجدولة المجمعة</label></div>
                <div className="flex items-center p-2"><input type="checkbox" id="limit-approval" checked={draftPlan.limits.contentApprovalWorkflow} onChange={e => handleLimitChange('contentApprovalWorkflow', e.target.checked)} className="h-4 w-4" /><label htmlFor="limit-approval" className="mr-2 whitespace-normal">workflow موافقة المحتوى</label></div>
                <div className="flex items-center p-2"><input type="checkbox" id="limit-analytics" checked={draftPlan.limits.deepAnalytics} onChange={e => handleLimitChange('deepAnalytics', e.target.checked)} className="h-4 w-4" /><label htmlFor="limit-analytics" className="mr-2 whitespace-normal">التحليلات العميقة</label></div>
                ...
                <div className="flex items-center p-2">
                  <input
                    ...
                    className="h-4 w-4"
                  />
                  <label htmlFor="adminOnly" className="mr-2 whitespace-normal">خاص بالمسؤولين فقط</label>
                </div>
