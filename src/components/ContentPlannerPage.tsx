...
const ContentPlannerPage: React.FC<ContentPlannerPageProps> = ({ 
  ...
  return (
        ...
        <form onSubmit={handleGeneratePlanSubmit} className="space-y-6">
            <div className="space-y-4 fade-in">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
                    <StrategyButton label="خطة قياسية" icon={<BrainCircuitIcon className="w-8 h-8" />} active={strategyType === 'standard'} onClick={() => setStrategyType('standard')} disabled={isViewer}/>
                    <StrategyButton label="حملة تسويقية" icon={<CalendarCheckIcon className="w-8 h-8" />} active={strategyType === 'campaign'} onClick={() => setStrategyType('campaign')} disabled={isViewer}/>
                    <StrategyButton label="حملة لمناسبة" icon={<CalendarCheckIcon className="w-8 h-8 text-green-500" />} active={strategyType === 'occasion'} onClick={() => setStrategyType('occasion')} disabled={isViewer}/>
                    <StrategyButton label="محتوى محوري" icon={<BrainCircuitIcon className="w-8 h-8" />} active={strategyType === 'pillar'} onClick={() => setStrategyType('pillar')} disabled={isViewer}/>
            ...
            <div className="space-y-6 fade-in border-t dark:border-gray-700 pt-6">
                ...
                {planDuration === 'monthly' && (
                    <div className="fade-in">
                        ...
                        <select id="monthlyPostCount" value={monthlyPostCount} onChange={(e) => setMonthlyPostCount(Number(e.target.value) as 8|12|16|30)} className="w-full p-2 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-700" disabled={isViewer}>
            ...
            {!isViewer && (
              <div className="pt-6 flex justify-center items-center">
                  <Button type="submit" size="lg" isLoading={isGenerating} disabled={!aiClient || isGenerating || isViewer}>
      ...
      {plan && (
        <div className="max-w-7xl mx-auto space-y-6">
           ...
           <div className="flex justify-center flex-wrap gap-4">
                {!isViewer && (
                  <Button
                      ...
                      disabled={isSchedulingStrategy || !aiClient || isViewer}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {plan.map((item, index) => (
              <ContentPlanCard key={index} item={item} onStartPost={isViewer ? () => {} : onStartPost} />
            ))}
          </div>
        </div>
      )}
    ...
    <StrategyHistoryModal 
        ...
        onDelete={isViewer ? () => {} : onDeleteFromHistory}
    />
    </>
  );
};
