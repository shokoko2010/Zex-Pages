...
export const useDashboardData = ({
    ...
    const syncFacebookData = useCallback(async (target: Target, lastSyncTime?: string) => {
        ...
        try {
            ...
            // Process more posts but still be mindful of API limits
            const maxPostsToProcess = Math.min(newPublishedPosts.length, 25); // Increased from 10 to 25
